/**
 * [REQ:P4-F05_1] Simple message passing
 * Goal: Nearby agents exchange messages with queue limits, read receipts, and
 * optional actions that apply when read. Out-of-range agents must not receive
 * or overhear the transmission.
 *
 * Verification per requirement:
 *  - unit: emit from one agent, ingest from another, observe queue behaviour,
...
(async () => {
  ...
  const expectedExports = {
    ...
    agent_observation_get_x: "function",
    agent_observation_get_y: "function",
    agent_emission_get_message_queue_capacity: "function",
    agent_emission_send: "function",
    agent_emission_receive_next: "function",
    agent_emission_poll_receipt: "function",
  };
  ...
  const sender = mod.agent_lifecycle_create();
  const receiver = mod.agent_lifecycle_create();
  const outsider = mod.agent_lifecycle_create();

  try {
    // Initialise agents and set positions
    mod.agent_lifecycle_init(sender);
    mod.agent_lifecycle_init(receiver);
    mod.agent_lifecycle_init(outsider);
    mod.agent_lifecycle_step(sender);
    mod.agent_lifecycle_step(receiver);
    mod.agent_lifecycle_step(outsider);

    mod.agent_transition_move_by(receiver, 3, 0);
    mod.agent_lifecycle_step(receiver);
    mod.agent_transition_move_by(outsider, 12, 0);
    mod.agent_lifecycle_step(outsider);
    ...
    // Out-of-range agents must not receive messages
    const outOfRangeId = mod.agent_emission_send(sender, outsider, radius, 0, 0, 42);
    assert.equal(outOfRangeId, 0, "message should not deliver outside radius");

    const capacity = mod.agent_emission_get_message_queue_capacity();
    const sentIds = [];

    // Saturate the receiver queue (capacity + 1 messages should drop the oldest)
    for (let i = 0; i < capacity + 1; i++) {
      const id = mod.agent_emission_send(sender, receiver, radius, 0, 0, 100 + i);
    ...
    const actionId = mod.agent_emission_send(sender, receiver, radius, 2, 1, 999);
    ...
    const received = [];
    while (true) {
      const message = mod.agent_emission_receive_next(receiver);
    ...
    const receipts = [];
    while (true) {
      const receipt = mod.agent_emission_poll_receipt(sender);
    ...
    const outsiderMsg = mod.agent_emission_receive_next(outsider);
    assert.strictEqual(outsiderMsg, null, "outsider should not see in-range messages");

    console.log("[REQ:P4-F05_1] simple message passing tests: ok");
  } finally {
